...
const DifficultyDropdown = () => {
  ...
  useEffect(() => {
    console.log(level !== previousLevel)
    if(level === null) {
      setLevel(difficulty);
    }
    else if(level !== previousLevel) setDifficulty(level);
    else {
      setLevel(difficulty);
      ...
      >
        {levels.map((item) => (
          <button
              ...
              className={`flex items-center self-stretch px-[10px] gap-3 pt-0 pb-[10px] text-[16px] flex-grow leading-[19.2px] tracking-[-0.48px] text-[#FFF] ${item === 'Hard' ? "" : "border-b border-[#3A3A3A]"}`}
            >
